...
- **`-march=native`** tunes the binary for the exact CPU it's compiled on (enables AVX2, SSE4, etc.). Only used for source installs — pre-built wheels omit it so they run on any CPU of that architecture.
...
- **`-ffast-math`** / **`/fp:fast`** allows the compiler to reorder floating-point operations for speed. This is appropriate here because tracking algorithms don't require strict IEEE 754 compliance.
