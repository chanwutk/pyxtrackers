#!/usr/bin/env python3
"""
Test suite for comparing tracking results from two OC-SORT implementations:
- polyis/tracker/ocsort/ocsort.py (Python implementation)
- polyis/tracker/ocsort/cython/ocsort.pyx (Cython implementation)
...
def test_ocsort_comparison():
    """
    Test comparing tracking results from Python and Cython OC-SORT implementations.
    ...
    if os.path.exists(tracker_config_path):
        ...
        with open(tracker_config_path, 'r') as f:
            tracker_config = yaml.safe_load(f).get('ocsort', {})
        ...
        det_thresh = tracker_config.get('det_thresh', 0.3)
    ...
    if perf_python and perf_cython:
        ...
        if perf_python['total_time'] > 0 and perf_cython['total_time'] > 0:
            ...
            if speedup_cython > 1.0:
                print(f"  → Cython OC-SORT is {speedup_cython:.2f}x faster than Python")
