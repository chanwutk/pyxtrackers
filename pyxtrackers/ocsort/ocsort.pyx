...
cdef void convert_bbox_to_z(double *bbox, double *z) noexcept nogil:
    ...
    # Ref: references/ocsort/ocsort.py#L21-L33 (convert_bbox_to_z)
    cdef double w = bbox[2] - bbox[0]  # Ref: references/ocsort/ocsort.py#L27 — w = bbox[2] - bbox[0]
    cdef double h = bbox[3] - bbox[1]  # Ref: references/ocsort/ocsort.py#L28 — h = bbox[3] - bbox[1]
    z[0] = bbox[0] + w / 2.0           # Ref: references/ocsort/ocsort.py#L29 — x = bbox[0] + w/2.
    z[1] = bbox[1] + h / 2.0           # Ref: references/ocsort/ocsort.py#L30 — y = bbox[1] + h/2.
    z[2] = w * h                        # Ref: references/ocsort/ocsort.py#L31 — s = w * h
    z[3] = w / (h + 1e-6)              # Ref: references/ocsort/ocsort.py#L32 — r = w / float(h+1e-6), uses 1e-6 epsilon
...
cdef void convert_x_to_bbox(double *x, double *bbox) noexcept nogil:
    ...
    # Ref: references/ocsort/ocsort.py#L36-L46 (convert_x_to_bbox)
    cdef double w = sqrt(x[2] * x[3])  # Ref: references/ocsort/ocsort.py#L41 — w = np.sqrt(x[2] * x[3])
    cdef double h = x[2] / w           # Ref: references/ocsort/ocsort.py#L42 — h = x[2] / w
    bbox[0] = x[0] - w / 2.0           # Ref: references/ocsort/ocsort.py#L44 — x[0]-w/2.
    bbox[1] = x[1] - h / 2.0           # Ref: references/ocsort/ocsort.py#L44 — x[1]-h/2.
    bbox[2] = x[0] + w / 2.0           # Ref: references/ocsort/ocsort.py#L44 — x[0]+w/2.
    bbox[3] = x[1] + h / 2.0           # Ref: references/ocsort/ocsort.py#L44 — x[1]+h/2.
...
cdef void speed_direction_internal(double *bbox1, double *bbox2, double *speed) noexcept nogil:
    ...
    # Ref: references/ocsort/ocsort.py#L49-L54 (speed_direction)
    cdef double cx1 = (bbox1[0] + bbox1[2]) / 2.0  # Ref: references/ocsort/ocsort.py#L50 — cx1
    cdef double cy1 = (bbox1[1] + bbox1[3]) / 2.0  # Ref: references/ocsort/ocsort.py#L50 — cy1
    cdef double cx2 = (bbox2[0] + bbox2[2]) / 2.0  # Ref: references/ocsort/ocsort.py#L51 — cx2
    cdef double cy2 = (bbox2[1] + bbox2[3]) / 2.0  # Ref: references/ocsort/ocsort.py#L51 — cy2
    cdef double dx = cx2 - cx1                      # Ref: references/ocsort/ocsort.py#L52 — cx2-cx1
    cdef double dy = cy2 - cy1                      # Ref: references/ocsort/ocsort.py#L52 — cy2-cy1
    cdef double norm = sqrt(dx * dx + dy * dy) + 1e-6  # Ref: references/ocsort/ocsort.py#L53 — norm
    speed[0] = dy / norm  # Ref: references/ocsort/ocsort.py#L54 — speed / norm, dy component
    speed[1] = dx / norm  # Ref: references/ocsort/ocsort.py#L54 — speed / norm, dx component
...
# Ref: references/ocsort/ocsort.py#L57-L103 (KalmanBoxTracker class)
# Includes observations dict, velocity, last_observation, delta_t.
cdef struct KalmanBoxTracker:
    KalmanFilter kf
    int time_since_update  # Ref: references/ocsort/ocsort.py#L87 — self.time_since_update
    int id                 # Ref: references/ocsort/ocsort.py#L88 — self.id
    int hits               # Ref: references/ocsort/ocsort.py#L91 — self.hits
    int hit_streak         # Ref: references/ocsort/ocsort.py#L92 — self.hit_streak
    int age                # Ref: references/ocsort/ocsort.py#L93 — self.age
    int delta_t            # Ref: references/ocsort/ocsort.py#L103 — self.delta_t
    # Ref: references/ocsort/ocsort.py#L99 — self.last_observation = np.array([-1, -1, -1, -1, -1])
    # Last observation [x1, y1, x2, y2, score] or [-1,...] placeholder
    double last_observation[5]
    # Ref: references/ocsort/ocsort.py#L102 — self.velocity = None
    # Velocity [dy, dx]
    double velocity[2]
    int has_velocity
    # Ref: references/ocsort/ocsort.py#L100 — self.observations = dict()
...
cdef void KalmanBoxTracker_init(KalmanBoxTracker *self, double *bbox, int id, int delta_t) noexcept nogil:
    ...
    # Ref: references/ocsort/ocsort.py#L63-L103 (KalmanBoxTracker.__init__)
    cdef int i, j

    # Initialize Kalman Filter
    # Ref: references/ocsort/ocsort.py#L69-L71 — self.kf = KalmanFilter(dim_x=7, dim_z=4)
    kf_init(&self.kf)
    # Ref: references/ocsort/ocsort.py#L75-L76 — self.kf.F (constant velocity model)
    # Constant velocity model
    self.kf.F[0][4] = 1.0
    self.kf.F[1][5] = 1.0
    self.kf.F[2][6] = 1.0
    # Ref: references/ocsort/ocsort.py#L77-L78 — self.kf.H (observation matrix)
    # Observation matrix
    self.kf.H[0][0] = 1.0
    self.kf.H[1][1] = 1.0
    self.kf.H[2][2] = 1.0
    self.kf.H[3][3] = 1.0
    # Ref: references/ocsort/ocsort.py#L80 — self.kf.R[2:, 2:] *= 10.
    # Adjust covariance matrices
    self.kf.R[2][2] *= 10.0
    self.kf.R[3][3] *= 10.0
    # Ref: references/ocsort/ocsort.py#L81 — self.kf.P[4:, 4:] *= 1000.
    self.kf.P[4][4] *= 1000.0
    self.kf.P[5][5] *= 1000.0
    self.kf.P[6][6] *= 1000.0
    # Ref: references/ocsort/ocsort.py#L82 — self.kf.P *= 10.
    for i in range(7):
        for j in range(7):
            self.kf.P[i][j] *= 10.0
    # Ref: references/ocsort/ocsort.py#L83 — self.kf.Q[-1, -1] *= 0.01
    self.kf.Q[6][6] *= 0.01
    # Ref: references/ocsort/ocsort.py#L84 — self.kf.Q[4:, 4:] *= 0.01
    self.kf.Q[4][4] *= 0.01
    self.kf.Q[5][5] *= 0.01
    self.kf.Q[6][6] *= 0.01

    # Ref: references/ocsort/ocsort.py#L86 — self.kf.x[:4] = convert_bbox_to_z(bbox)
    # Initialize state with bbox
    convert_bbox_to_z(bbox, self.kf.x)

    # Ref: references/ocsort/ocsort.py#L87-L93 — state fields
    # State fields
    ...
    self.hit_streak = 0
    self.age = 0
    # Ref: references/ocsort/ocsort.py#L103 — self.delta_t = delta_t
...
cdef void KalmanBoxTracker_update(KalmanBoxTracker *self, double *bbox) noexcept nogil:
    ...
    # Ref: references/ocsort/ocsort.py#L105-L138 (KalmanBoxTracker.update)
    ...
    if bbox != NULL:
        # Ref: references/ocsort/ocsort.py#L109-L110 — if bbox is not None, check last_observation
        # Velocity estimation from previous observation
        # Ref: references/ocsort/ocsort.py#L110 — if self.last_observation.sum() >= 0
        if self.last_observation[0] >= 0:
            found = 0
            # Ref: references/ocsort/ocsort.py#L112-L116 — search delta_t steps back for previous observation
            for i in range(self.delta_t):
                ...
                if self.age - dt >= 0:
                    ...
                    if self.age - dt < self.observations_capacity:
                        ...
                        if sum_val > 1e-9:
                            # Ref: references/ocsort/ocsort.py#L115-L116 — previous_box = self.observations[self.age-dt]
                            # Found valid observation, convert from stored bbox to speed dir
                            # Ref: references/ocsort/ocsort.py#L122 — self.velocity = speed_direction(previous_box, bbox)
                            speed_direction_internal(&self.observations[obs_idx], bbox, self.velocity)
                            self.has_velocity = 1
                            found = 1
                            break
            if not found:
                # Ref: references/ocsort/ocsort.py#L117-L118 — if previous_box is None: previous_box = self.last_observation
                # Ref: references/ocsort/ocsort.py#L122 — self.velocity = speed_direction(previous_box, bbox)
                speed_direction_internal(self.last_observation, bbox, self.velocity)
                self.has_velocity = 1

        # Ref: references/ocsort/ocsort.py#L128-L130 — store observation and update last_observation
        # Store observation in array
        _ensure_obs_capacity(self, self.age)
        obs_idx = self.age * 5
        # Ref: references/ocsort/ocsort.py#L129 — self.observations[self.age] = bbox
        ...
        # Ref: references/ocsort/ocsort.py#L128 — self.last_observation = bbox
        # Update last observation
        for i in range(5):
            self.last_observation[i] = bbox[i]

        # Ref: references/ocsort/ocsort.py#L132-L135 — update counters
        # Update counters
        self.time_since_update = 0
        self.hits += 1
        self.hit_streak += 1

        # Ref: references/ocsort/ocsort.py#L136 — self.kf.update(convert_bbox_to_z(bbox))
        # Update Kalman filter
        convert_bbox_to_z(bbox, z)
        kf_update(&self.kf, z)
    else:
        # Ref: references/ocsort/ocsort.py#L137-L138 — self.kf.update(bbox) where bbox is None
        # No observation
...
cdef void KalmanBoxTracker_predict(KalmanBoxTracker *self, double *bbox) noexcept nogil:
    ...
    # Ref: references/ocsort/ocsort.py#L140-L153 (KalmanBoxTracker.predict)
    # Ref: references/ocsort/ocsort.py#L144-L145 — clamp negative scale
    if (self.kf.x[6] + self.kf.x[2]) <= 0:
        self.kf.x[6] = 0.0
    # Ref: references/ocsort/ocsort.py#L147 — self.kf.predict()
    kf_predict(&self.kf)
    # Ref: references/ocsort/ocsort.py#L148 — self.age += 1
    self.age += 1
    # Ref: references/ocsort/ocsort.py#L149-L150 — reset hit_streak if time_since_update > 0
    if self.time_since_update > 0:
        self.hit_streak = 0
    # Ref: references/ocsort/ocsort.py#L151 — self.time_since_update += 1
    self.time_since_update += 1
    # Ref: references/ocsort/ocsort.py#L152 — convert_x_to_bbox(self.kf.x)
...
cdef void KalmanBoxTracker_get_state(KalmanBoxTracker *self, double *bbox) noexcept nogil:
    ...
    # Ref: references/ocsort/ocsort.py#L155-L159 (KalmanBoxTracker.get_state)
    # Ref: references/ocsort/ocsort.py#L159 — return convert_x_to_bbox(self.kf.x)
    convert_x_to_bbox(self.kf.x, bbox)
...
cdef void k_previous_obs(KalmanBoxTracker *trk, double *result) noexcept nogil:
    ...
    # Ref: references/ocsort/ocsort.py#L10-L18 (k_previous_obs function)
    cdef int i, j, dt, obs_idx
    cdef double sum_val
    cdef int max_age_idx

    # Ref: references/ocsort/ocsort.py#L11-L12 — if len(observations) == 0: return [-1,...,-1]
    if trk.obs_ages_len == 0:
        for i in range(5):
            result[i] = -1.0
        return

    # Ref: references/ocsort/ocsort.py#L13-L16 — search delta_t steps back
    # Try to find observation delta_t steps ago
    for i in range(trk.delta_t):
        dt = trk.delta_t - i
        # Ref: references/ocsort/ocsort.py#L15 — if cur_age - dt in observations
        if trk.age - dt >= 0 and trk.age - dt < trk.observations_capacity:
            ...
            if sum_val > 1e-9:
                # Ref: references/ocsort/ocsort.py#L16 — return observations[cur_age-dt]
                for j in range(5):
                    result[j] = trk.observations[obs_idx + j]
                return

    # Ref: references/ocsort/ocsort.py#L17-L18 — fallback: max_age = max(observations.keys()); return observations[max_age]
...
# Ref: references/ocsort/ocsort.py#L175-L191 (OCSort class)
cdef class OCSort:
    ...
    def __init__(self, det_thresh, max_age=30, min_hits=3,
                 ...
                 use_byte=False):
        # Ref: references/ocsort/ocsort.py#L176-L191 (OCSort.__init__)
        # Ref: references/ocsort/ocsort.py#L181 — self.max_age = max_age
        self._max_age = max_age
        # Ref: references/ocsort/ocsort.py#L182 — self.min_hits = min_hits
        self._min_hits = min_hits
        # Ref: references/ocsort/ocsort.py#L183 — self.iou_threshold = iou_threshold
        self._iou_threshold = iou_threshold
        # Ref: references/ocsort/ocsort.py#L186 — self.det_thresh = det_thresh
        self._det_thresh = det_thresh
        # Ref: references/ocsort/ocsort.py#L187 — self.delta_t = delta_t
        self._delta_t = delta_t
        # Ref: references/ocsort/ocsort.py#L189 — self.inertia = inertia
        self._inertia = inertia
        # Ref: references/ocsort/ocsort.py#L190 — self.use_byte = use_byte
        self._use_byte = 1 if use_byte else 0
        # Ref: references/ocsort/ocsort.py#L188 — self.asso_func = ASSO_FUNCS[asso_func]
    ...
    cdef int _update_c(self, double *dets_all, double *scores_all, int n_all,
        ...
        """
        # Ref: references/ocsort/ocsort.py#L193-L325 (OCSort.update method)
        ...
        # Ref: references/ocsort/ocsort.py#L204 — self.frame_count += 1
        self._frame_count += 1

        # Build dets array (Nx5: x1,y1,x2,y2,score)
        # Ref: references/ocsort/ocsort.py#L216-L222 — build dets from bboxes and scores, filter by det_thresh
        n_dets = n_all
        ...
        # Ref: references/ocsort/ocsort.py#L219-L220 — dets_second for low-score detections
        ...
        # Ref: references/ocsort/ocsort.py#L224-L235 — predict all trackers, remove NaN
        n_trks = <int>self._trackers.size()
        trks = <double *>calloc((n_trks + 1) * 5, sizeof(double))
        to_del = <int *>malloc((n_trks + 1) * sizeof(int))

        # Ref: references/ocsort/ocsort.py#L228-L232 — predict each tracker, check for NaN
        for t in range(n_trks):
            KalmanBoxTracker_predict(self._trackers[t], pred_bbox)
            # Ref: references/ocsort/ocsort.py#L230 — trk[:] = [pos[0], pos[1], pos[2], pos[3], 0]
            trks[t * 5 + 0] = pred_bbox[0]
            trks[t * 5 + 1] = pred_bbox[1]
            trks[t * 5 + 2] = pred_bbox[2]
            trks[t * 5 + 3] = pred_bbox[3]
            trks[t * 5 + 4] = 0.0
            # Ref: references/ocsort/ocsort.py#L231-L232 — if np.any(np.isnan(pos)): to_del.append(t)
            has_nan = 0
            for k in range(4):
        ...
        # Ref: references/ocsort/ocsort.py#L234-L235 — remove NaN trackers in reverse order
        ...
        # Ref: references/ocsort/ocsort.py#L237-L241
        ...
        for t in range(n_trks):
            # Ref: references/ocsort/ocsort.py#L237-L238 — velocities array
            if self._trackers[t].has_velocity:
                velocities[t * 2 + 0] = self._trackers[t].velocity[0]
                velocities[t * 2 + 1] = self._trackers[t].velocity[1]
            # Ref: references/ocsort/ocsort.py#L239 — last_boxes = [trk.last_observation for ...]
            for k in range(5):
                last_boxes[t * 5 + k] = self._trackers[t].last_observation[k]
            # Ref: references/ocsort/ocsort.py#L240-L241 — k_observations = [k_previous_obs(...) for ...]
            k_previous_obs(self._trackers[t], &k_obs[t * 5])

        # ---- First round association ----
        # Ref: references/ocsort/ocsort.py#L246-L249 — first round: associate(dets, trks, ...)
        ...
        # Ref: references/ocsort/ocsort.py#L246-L247 — matched, unmatched_dets, unmatched_trks = associate(...)
        ...
        # Ref: references/ocsort/ocsort.py#L248-L249 — for m in matched: self.trackers[m[1]].update(dets[m[0], :])
        ...
        # Ref: references/ocsort/ocsort.py#L254-L273 — BYTE association with low-score dets
        if self._use_byte and n_second > 0 and n_unmatched_t > 0:
            # Ref: references/ocsort/ocsort.py#L256 — u_trks = trks[unmatched_trks]
            n_ut = n_unmatched_t
            ...
            # Ref: references/ocsort/ocsort.py#L257 — iou_left = self.asso_func(dets_second, u_trks)
            ...
            # Ref: references/ocsort/ocsort.py#L259 — if iou_left.max() > self.iou_threshold
            max_iou = 0.0
            ...
            if max_iou > self._iou_threshold:
                # Ref: references/ocsort/ocsort.py#L265 — matched_indices = linear_assignment(-iou_left)
                ...
                # Ref: references/ocsort/ocsort.py#L267-L272 — filter by threshold, update matched trackers
                to_remove_trk = <int *>malloc((n_bm + 1) * sizeof(int))
                n_remove_trk = 0
                for i in range(n_bm):
                    d = bm_a[i]
                    j = bm_b[i]
                    # Ref: references/ocsort/ocsort.py#L269-L270 — if iou_left[m[0], m[1]] < threshold: continue
                    if iou_left[d * n_ut + j] >= self._iou_threshold:
                        t = unmatched_t[j]
                        for k in range(5):
                            det_bbox[k] = dets_sec[d * 5 + k]
                        # Ref: references/ocsort/ocsort.py#L271 — self.trackers[trk_ind].update(dets_second[det_ind, :])
                        KalmanBoxTracker_update(self._trackers[t], det_bbox)
                ...
                # Ref: references/ocsort/ocsort.py#L273 — unmatched_trks = np.setdiff1d(...)
        ...
        # Ref: references/ocsort/ocsort.py#L275-L297 — re-association using last_boxes
        if n_unmatched_d > 0 and n_unmatched_t > 0:
            n_ld = n_unmatched_d
            n_lt = n_unmatched_t
            # Ref: references/ocsort/ocsort.py#L276 — left_dets = dets[unmatched_dets]
            left_det_bb = <double *>malloc(n_ld * 4 * sizeof(double))
            # Ref: references/ocsort/ocsort.py#L277 — left_trks = last_boxes[unmatched_trks]
            ...
            # Ref: references/ocsort/ocsort.py#L278 — iou_left = self.asso_func(left_dets, left_trks)
            ...
            # Ref: references/ocsort/ocsort.py#L280 — if iou_left.max() > self.iou_threshold
            max_iou2 = 0.0
            ...
            if max_iou2 > self._iou_threshold:
                # Ref: references/ocsort/ocsort.py#L286 — rematched_indices = linear_assignment(-iou_left)
                ...
                # Ref: references/ocsort/ocsort.py#L289-L295 — filter by threshold, update matched
                to_rm_d = <int *>malloc((n_om + 1) * sizeof(int))
                to_rm_t2 = <int *>malloc((n_om + 1) * sizeof(int))
                n_rm_d = 0
                n_rm_t2 = 0
                for i in range(n_om):
                    # Ref: references/ocsort/ocsort.py#L291 — if iou_left[m[0], m[1]] < threshold: continue
                    if iou_left2[om_a[i] * n_lt + om_b[i]] >= self._iou_threshold:
                        # Ref: references/ocsort/ocsort.py#L290 — det_ind, trk_ind = unmatched_dets[m[0]], unmatched_trks[m[1]]
                        d = unmatched_d[om_a[i]]
                        t = unmatched_t[om_b[i]]
                        for k in range(5):
                            det_bbox[k] = dets[d * 5 + k]
                        # Ref: references/ocsort/ocsort.py#L293 — self.trackers[trk_ind].update(dets[det_ind, :])
                        KalmanBoxTracker_update(self._trackers[t], det_bbox)
                        to_rm_d[n_rm_d] = d
                        n_rm_d += 1
                        to_rm_t2[n_rm_t2] = t
                ...
                # Ref: references/ocsort/ocsort.py#L296 — unmatched_dets = np.setdiff1d(...)
                ...
                # Ref: references/ocsort/ocsort.py#L297 — unmatched_trks = np.setdiff1d(...)
        ...
        # Ref: references/ocsort/ocsort.py#L299-L300 — for m in unmatched_trks: self.trackers[m].update(None)
        for i in range(n_unmatched_t):
            KalmanBoxTracker_update(self._trackers[unmatched_t[i]], NULL)

        # ---- Create new trackers for unmatched detections ----
        # Ref: references/ocsort/ocsort.py#L302-L305 — for i in unmatched_dets: trk = KalmanBoxTracker(dets[i,:])
        for i in range(n_unmatched_d):
            d = unmatched_d[i]
        ...
        # Ref: references/ocsort/ocsort.py#L307-L322
        i = <int>self._trackers.size()
        for idx in range(<int>self._trackers.size() - 1, -1, -1):
            trk_ptr = self._trackers[idx]
            i -= 1

            # Ref: references/ocsort/ocsort.py#L308-L315 — use last_observation or KF state
            obs_sum = 0.0
            for k in range(5):
                obs_sum += trk_ptr.last_observation[k]
            # Ref: references/ocsort/ocsort.py#L308 — if trk.last_observation.sum() < 0: d = trk.get_state()[0]
            if obs_sum < 0:
                KalmanBoxTracker_get_state(trk_ptr, state_bbox)
            else:
                # Ref: references/ocsort/ocsort.py#L315 — d = trk.last_observation[:4]
                for k in range(4):
                    state_bbox[k] = trk_ptr.last_observation[k]

            # Ref: references/ocsort/ocsort.py#L316 — if (trk.time_since_update < 1) and (trk.hit_streak >= self.min_hits or self.frame_count <= self.min_hits)
            if trk_ptr.time_since_update < 1 and (trk_ptr.hit_streak >= self._min_hits or self._frame_count <= self._min_hits):
                ...
                # Ref: references/ocsort/ocsort.py#L318 — [trk.id+1] — +1 for MOT IDs (benchmark requires positive)
                output[n_output * 5 + 4] = <double>(trk_ptr.id + 1)
                n_output += 1

            # Ref: references/ocsort/ocsort.py#L321-L322 — if trk.time_since_update > self.max_age: self.trackers.pop(i)
    ...
    def update(self, dets):
        ...
        # Ref: references/ocsort/ocsort.py#L217-L222 — score filtering
